...
class MedicMedicalBill(models.Model):
    ...
    def _compute_phanl_loai_char(self):
        a = [x.id for x in (self.env.ref('dham_medic.%s'%(r)) for r in ['loai_1', 'loai_2', 'loai_3'])]
        b = [x.id for x in (self.env.ref('dham_medic.%s'%(r)) for r in ['loai_4', 'loai_5'])]
        for record in self:
    ...
    def _get_medic_test_ids(self):
        MedictTest = self.env['medic.test']
        lab_type = [self.env.ref('dham_medic.medic_test_type_lab_test').id]
        image_type = [self.env.ref('dham_medic.medic_test_type_image_test').id,
                      self.env.ref('dham_medic.medic_test_type_echograph').id,
                      self.env.ref('dham_medic.medic_test_type_electrocardiogram').id]
        for record in self:
            test_ids = MedictTest.search([('related_medical_bill', 'in', [record.id])])
    ...
    def parse_sub_treatment(self):
        ...
        for khoa in khoas:
            if khoa == 'khoa_san' and self.sex == 'male':
                continue
            res.append((0, 0, {'buildings_type': self.env.ref('dham_medic.' + khoa).id,
                               'doctor_id': False,
                               'type': 'company',
                               }))
        return res
...
class MedicSubTreatment(models.Model):
    ...
    def _compute_check_show_result(self):
        for record in self:
            ...
            try:
                if record.buildings_type.id == self.env.ref('dham_medic.khoa_nhi').id:
                    record.check_nhi = True
                elif record.buildings_type.id == self.env.ref('dham_medic.khoa_mat').id:
                    record.check_mat = True
                elif record.buildings_type.id == self.env.ref('dham_medic.khoa_tai_mui_hong').id:
                    record.check_tai_mui_hong = True
                elif record.buildings_type.id == self.env.ref('dham_medic.khoa_rang_ham_mat').id:
                    record.check_rang_ham_mat = True
                elif record.buildings_type.id == self.env.ref('dham_medic.khoa_da_lieu').id:
                    record.check_da_lieu = True
                elif record.buildings_type.id == self.env.ref('dham_medic.khoa_ngoai').id:
                    record.check_ngoai = True
                elif record.buildings_type.id == self.env.ref('dham_medic.khoa_san').id:
