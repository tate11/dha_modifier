    ...
    def onchange_order_type(self):
        if self.order_type == 'medicine':
            try:
                return {
                    'domain': {
                        'product_id': [('categ_id', '=', self.env.ref('dha_medic_modifier.product_ctg_medicines').id)]
                    }
                }
            except:
                pass
        if self.order_type == 'food-drink':
            try:
                return {
                    'domain': {
                        'product_id': [('categ_id', '=', self.env.ref('dha_medic_modifier.product_ctg_food_drink').id)]
                    }
                }
            except:
                pass